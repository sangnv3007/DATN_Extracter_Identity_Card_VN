...
def process():
    ...
    process_bar = ttk.Progressbar(page2, orient = HORIZONTAL, length = 200, mode = 'determinate')
...
def res():
    ...
    #
    num_face_input, num_face_selfie, face_distance = compare(pathFront, pathFace)
    print(num_face_input, num_face_selfie, face_distance)
    if(face_distance is not None):
        res_from_distance = face_distance < 0.55
        lb_res_facematch.configure(text='{} ({})'.format(res_from_distance,face_confidence(face_distance)))
        ...
        messagebox.showerror('Thông báo', 'Ảnh selfie hoặc ảnh CCCD không hợp lệ.')
# Create formFp
formF = Tk()
...
show_frame(page1)
### Image Icon ###
image_uploadFile = customtkinter.CTkImage(Image.open('./images/upload.png'), size=(30, 30))
image_continue = customtkinter.CTkImage(Image.open('./images/right-arrow.png'), size=(30, 30))
image_back = customtkinter.CTkImage(Image.open('./images/back.png'), size=(30, 30))
image_ocr = customtkinter.CTkImage(Image.open('./images/ocr1.png'), size=(30, 30))
image_facematch= customtkinter.CTkImage(Image.open('./images/face-recognition.png'), size=(30, 30))
...
bt_uploadF = customtkinter.CTkButton(master=page1,
                                 ...
                                 text_color= 'white',
                                 font=customtkinter.CTkFont(family="Times",size=18, weight="bold"),
                                 width=600,
                                 height=50,                                
                                 corner_radius=10, 
                                 fg_color= '#2596be', 
...
res_photo_front = ImageTk.PhotoImage(Image.open("cmt.be3f6567.png").resize((250, 200)))
lb_res_front= Label(page4,image=res_photo_front)
lb_res_front.place(relx=0.2,rely=0.2, anchor= CENTER)
res_photo_back = ImageTk.PhotoImage(Image.open("cmt_back.29611820.png").resize((250, 200)))
lb_res_back= Label(page4,image=res_photo_back)
lb_res_back.place(relx=0.5, rely=0.2, anchor=CENTER)
res_photo_face = ImageTk.PhotoImage(Image.open("face.c8f1db03.png").resize((250, 200)))
